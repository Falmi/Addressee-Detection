...
#from compat_BP import CompactBilinearPooling
...
class talkNetModel(nn.Module):
    def __init__(self):
        ...
        #self.CBP=CompactBilinearPooling(128,128,256)
        #CBP.cuda()
        #CBP.train()

        self.BLP = nn.Bilinear(128, 128, 256)
        # self.BLP = nn.Bilinear(256, 256, 256)

    def forward_visual_frontend(self, x):
        # print(f'before reshaped visual input shape {x.shape}')
        B, T, W, H = x.shape
        x = x.view(B * T, 1, 1, W, H)
        # print(f' after reshed for visual input shape {x.shape}')
    ...
    def forward_audio_frontend(self, x):
        # print(f'before reshaped audio input shape {x.shape}')
        x = x.unsqueeze(1).transpose(2, 3)
        # print(f'after reshaped audio input shape {x.shape}')
    ...
    # def forward_audio_visual_backend_org(self, x1, x2):
    #     #x = torch.cat((x1,x2), 2)
    ...
    def forward_audio_visual_backend(self, x1, x2):
        ...
        # comment this line to evaluate with out self attention
        x =self.selfAV(src=x,tar=x)
        x = torch.reshape(x, (-1, 256))
        return x
